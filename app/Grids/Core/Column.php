...
use DateTime;
use DateTimeZone;
use VotingSystem\Grids\Core;
use VotingSystem\Grids\Core\Filter;
use VotingSystem\Grids\Core\SearchFilter;
use VotingSystem\Grids\Core\SelectFilter;
use VotingSystem\Grids\Core\DateTimeRangeFilter;
...
class Column
{
    ...
    public function isDateWithoutTime($format = 'd M, Y')
    {
        $this->setCallback(function ($val) use ($format) {
            return (new DateTime($val))->format($format);
        });
    ...
    {
        $this->config->addStyleAttribute($name, $value);
        return $this;
    }
}
